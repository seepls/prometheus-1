...
Outer:
	for _, c := range cases.queries {
		...
		res, ws, err := querier.Select(storage.SelectParams{}, c.ms...)
		testutil.Ok(t, err)
	...
	for _, c := range cases {
		for _, overlapPercentage := range c.overlapPercentages {
			...
			b.Run(benchMsg, func(b *testing.B) {
				...
				ss, ws, err := sq.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchRegexp, "__name__", ".*"))
				for ss.Next() {
					it := ss.At().Iterator()
					for t := mint; t <= maxt; t++ {
						it.Seek(t)
...
func BenchmarkSetMatcher(b *testing.B) {
	...
	for _, c := range cases {
		...
		b.Run(benchMsg, func(b *testing.B) {
			...
			b.ReportAllocs()
			for n := 0; n < b.N; n++ {
				_, ws, err := que.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchRegexp, "test", c.pattern))
				testutil.Ok(b, err)
				testutil.Equals(b, 0, len(ws))
			}
		})
...
func benchQuery(b *testing.B, expExpansions int, q storage.Querier, selectors labels.Selector) {
	...
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		ss, ws, err := q.Select(storage.SelectParams{}, selectors...)
		testutil.Ok(b, err)
		testutil.Equals(b, 0, len(ws))
