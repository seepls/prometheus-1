...
func query(t testing.TB, q storage.Querier, matchers ...*labels.Matcher) map[string][]tsdbutil.Sample {
	ss, ws, err := q.Select(storage.SelectParams{}, matchers...)
	defer func() {
...
Outer:
	for _, c := range cases {
		...
		res, ws, err := q.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "a", "b"))
		testutil.Ok(t, err)
		testutil.Equals(t, 0, len(ws))
...
func TestDB_Snapshot(t *testing.T) {
	...
	// sum values
	seriesSet, ws, err := querier.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	testutil.Equals(t, 0, len(ws))
...
func TestDB_e2e(t *testing.T) {
	...
	for _, qry := range queries {
		...
		for i := 0; i < numRanges; i++ {
			...
			ss, ws, err := q.Select(storage.SelectParams{}, qry.ms...)
			testutil.Ok(t, err)
			testutil.Equals(t, 0, len(ws))
...
func TestNotMatcherSelectsLabelsUnsetSeries(t *testing.T) {
	...
	for _, c := range cases {
		ss, ws, err := q.Select(storage.SelectParams{}, c.selector...)
		testutil.Ok(t, err)
		testutil.Equals(t, 0, len(ws))
...
func TestDBReadOnly_FlushWAL(t *testing.T) {
	...
	// Sum the values.
	seriesSet, ws, err := querier.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, defaultLabelName, "flush"))
	testutil.Ok(t, err)
	testutil.Equals(t, 0, len(ws))
...
func TestDBCannotSeePartialCommits(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		func() {
			...
			ss, _, err := querier.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
			testutil.Ok(t, err)
...
func TestDBQueryDoesntSeeAppendsAfterCreation(t *testing.T) {
	...
	ss, _, err := querierBeforeAdd.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	_, seriesSet, err := expandSeriesSet(ss)
	testutil.Ok(t, err)
	testutil.Equals(t, map[string][]sample{}, seriesSet)

	ss, _, err = querierAfterAddButBeforeCommit.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	_, seriesSet, err = expandSeriesSet(ss)
	testutil.Ok(t, err)
	testutil.Equals(t, map[string][]sample{}, seriesSet)
	...
	ss, _, err = querierAfterCommit.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "foo", "bar"))
	testutil.Ok(t, err)
	_, seriesSet, err = expandSeriesSet(ss)
	testutil.Ok(t, err)
