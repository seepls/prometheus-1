		...
		Outer:
			for _, c := range cases {
				...
				for _, h := range []*Head{head, reloadedHead} {
					...
					testutil.Ok(t, err)
					actSeriesSet, ws, err := q.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, lblDefault.Name, lblDefault.Value))
					testutil.Ok(t, err)
...
func TestDelete_e2e(t *testing.T) {
	...
	for _, del := range dels {
		...
		for i := 0; i < numRanges; i++ {
			...
			testutil.Ok(t, err)
			defer q.Close()
			ss, ws, err := q.Select(storage.SelectParams{SeriesSorted: true}, del.ms...)
			testutil.Ok(t, err)
			testutil.Equals(t, 0, len(ws))
...
func TestUncommittedSamplesNotLostOnTruncate(t *testing.T) {
	...
	ss, ws, err := q.Select(storage.SelectParams{}, labels.MustNewMatcher(labels.MatchEqual, "a", "1"))
	testutil.Ok(t, err)
	testutil.Equals(t, 0, len(ws))
...
func TestHeadSeriesWithTimeBoundaries(t *testing.T) {
	...
	for i, c := range cases {
		...
		seriesCount := 0
		samplesCount := 0
		ss, _, err := q.Select(storage.SelectParams{}, matcher)
		testutil.Ok(t, err)
		for ss.Next() {
			i := ss.At().Iterator()
			for i.Next() {
